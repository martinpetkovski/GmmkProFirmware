...
void select_backlight_color(void) {
    if (backlight_color_index < 10) {
        set_backlight_color(backlight_colors[backlight_color_index].r, backlight_colors[backlight_color_index].g, backlight_colors[backlight_color_index].b);
    }
}
...
void add_backlight_color(uint8_t index, uint8_t r, uint8_t g, uint8_t b) {
    if (index >= 10) return;
    backlight_colors[index].r = r;
    backlight_colors[index].g = g;
    backlight_colors[index].b = b;
}
...
void keyboard_post_init_user(void) {
    ...
    add_backlight_color(0, 207, 171, 77);
    add_backlight_color(1, 213, 153, 142);
    add_backlight_color(2, 26, 72, 70);
    add_backlight_color(3, 202, 33, 40);
    add_backlight_color(4, 253, 212, 6);
    add_backlight_color(5, 0xFF, 0, 0);
    add_backlight_color(6, 0, 0xFF, 0);
    ...
    switch (active_layer) {
        ...
        case MP_FUNCTIONAL:
            ...
            for (uint32_t i = 0; i < 10; i++) {
                rgb_matrix_set_color(LED_LIST_NUMROW[i + 1], backlight_colors[i].r, backlight_colors[i].g, backlight_colors[i].b);
            }
